...
import { allUsers, allTournaments, editTournament, profile } from "../../redux/actions";
...
const EditTournamentDashboard = ({ id }) => {
    ...
    const viewEditTournament = dbTournament.find(edit => edit.id === id);
    ...
    useEffect(() => {
        const tournamentToUpdate = dbTournament.find(edit => edit.id === id);
        if (tournamentToUpdate) {
            setName(tournamentToUpdate.name);
            setParticipants(tournamentToUpdate.total_participants);
            setStartDate(tournamentToUpdate.start_date);
            setPrize(tournamentToUpdate.winner_prize);
            setCountry(tournamentToUpdate.country_of_location);
            setPrice(tournamentToUpdate.subscription_price);
            setProfilePictureUrl(tournamentToUpdate.cover_image);
        }
    }, [id, dbTournament]);
    ...
    const handlecreateTournament = () => {
        ...
        const originalName = viewEditTournament.name;
        const tournamentName = dbTournament.some((tournament) => tournament.name.toLowerCase() === name.toLowerCase());

        if (name !== originalName && tournamentName) {
      ...
      setErrorMessage(""); 
      setSuccessMessage("Successfully edit tournament");
      dispatch(editTournament(tournamentData, id));
    }
  ...
  return (
        ...
        <h1 className={styles.titleEditUserFull}>Edit Tournt</h1>
            ...
            <div>
                <button type="button" onClick={handlecreateTournament} className={styles.buttonEditUserContainerTwo} ><span>Edit Tournament</span></button>
...
export default EditTournamentDashboard